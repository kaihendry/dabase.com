...
## 13. Verbosity drowns the signal — cap the diff, not just the model
...
Agents don't just generate code fast — they generate a lot of it: more files touched, more lines added, more prose in the PR description, more comments restating what a well-named function already says. Every extra line is a small tax on whoever has to review it, and the human-bandwidth bottleneck [from point 1](#1-too-many-humans-in-the-loop) gets worse, not better, the bigger the diff a reviewer has to hold in their head at once. Succinct, load-bearing decisions get buried under boilerplate the model was happy to produce because tokens are cheap for it and expensive for the human reading them — cognitive overload, not lack of talent, is why a 900-line PR gets rubber-stamped instead of actually reviewed.

The fix isn't a style guide nobody reads, it's a sensor that blocks the noise before it lands. I use [sloc-sensor](https://github.com/kaihendry/sloc-sensor), pre-commit hooks that cap staged lines added — 100 by default, both in total and per file — plus absolute file length and even commit message length. It measures staged changes only, so deleting a bloated file is always free; the limit only bites on the way in, which is exactly where an agent's verbosity shows up. [Martin Fowler's piece on sensors for coding agents](https://martinfowler.com/articles/sensors-for-coding-agents.html) makes the underlying case: a hard, mechanical constraint holds where asking an agent nicely to be concise doesn't.

This only works as a habit if it isn't just personal. One developer capping their own diffs while everyone else lets the agent write 900-line PRs still leaves reviewers drowning — cognitive overload doesn't average out across a team, it's set by the worst offender in the pile. The sensor belongs in every repo's `.pre-commit-config.yaml`, not just the ones the one disciplined engineer happens to touch.
